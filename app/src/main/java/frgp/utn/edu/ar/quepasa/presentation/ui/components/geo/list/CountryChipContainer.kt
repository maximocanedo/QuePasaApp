...
fun GeoChipContainer(
    data: Set<Country>,
    onUnselectRequest: (Country) -> Unit,
    ...
    LazyRow(state = lazyListState) {
        items(items = data.take(visibleItems), key = { it.iso3 }) { state ->
            InputChip(
                selected = true,
                modifier = Modifier.padding(horizontal = 6.dp, vertical = 3.dp),
                onClick = { onUnselectRequest(state) },
    ...
    if(showingModal) ModalBottomSheet(
        ...
        if(data.isEmpty()) Text(
            text = "Cuando seleccionés países, van a aparecer acá. ",
            style = MaterialTheme.typography.bodySmall,
            textAlign = TextAlign.Center,
            modifier = Modifier
