...
@Composable
fun GeoChipContainer(
    data: Set<SubnationalDivision>,
    onUnselectRequest: (SubnationalDivision) -> Unit,
    ...
    LazyRow(state = lazyListState) {
        items(items = data.take(visibleItems), key = { it.iso3 }) { city ->
            InputChip(
                selected = true,
    ...
    if(showingModal) ModalBottomSheet(
        ...
        if(data.isEmpty()) Text(
            text = "Cuando seleccionés regiones, van a aparecer acá. ",
            style = MaterialTheme.typography.bodySmall,
            textAlign = TextAlign.Center,
            modifier = Modifier
